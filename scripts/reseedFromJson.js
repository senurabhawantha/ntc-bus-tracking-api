...
// const redacted = uri.replace(/\/\/([^:]+):([^@]+)@/, (m, u, p) => `//${u}:<redacted>@`);
// console.log('Connecting with URI:', redacted);
...
//     const fp = path.join(__dirname, '..', 'data', 'busSimulation.json');
//     const data = JSON.parse(fs.readFileSync(fp, 'utf-8'));
//     const routes = data.routes;
//     const buses = data.buses;

//     console.log('Clearing collections…');
//     await Promise.all([Bus.deleteMany({}), Route.deleteMany({})]);

...
const API_BASE = 'https://expresstravels.up.railway.app'; // <-- Railway URL
...
function drawPolylines(){
  ...
  if(currentRouteId==='all'){
    for(const rid in routesCoordinates){
      L.polyline(routesCoordinates[rid],{color:'blue',weight:5,opacity:0.4}).addTo(polylineGroup);
    }
  } else {
...
async function openDetails(bus_id){
  try{
    ...
    if(typeof lat==='number' && typeof lng==='number'){
      try{
        const geo = await fetch(`${API_BASE}/geocode?lat=${lat}&lng=${lng}`);
        if(geo.ok){
    ...
    panelBody().innerHTML=`
      ...
      <div class="details-row"><span>Last Updated:</span><strong>${new Date(bus.last_updated).toLocaleTimeString()}</strong></div>
      <div class="details-row"><span>City (approx):</span><strong>${city}</strong></div>
      <div class="details-row"><span>Coordinates:</span><strong>${lat?.toFixed?.(5)||lat||'-'}, ${lng?.toFixed?.(5)||lng||'-'}</strong></div>
...
async function animateBuses(routeBuses){
  ...
  for(const id of Object.keys(busData)){
    if(!keepIds.has(Number(id))){
      if(busData[id].marker) markerGroup.removeLayer(busData[id].marker);
      delete busData[id];
    }
  }
  ...
  routeBuses.forEach(bus=>{
    ...
    if(!busData[bus.bus_id].marker){
      const m=L.marker([lat,lng],{icon}).bindPopup(`<strong>Bus ${bus.bus_id}</strong><br>Status: <span style="color:${bus.status==='On Time'?'green':'red'}">${bus.status}</span><br><em>Click marker or row for details</em>`);
      m.on('click',()=>openDetails(bus.bus_id));
      busData[bus.bus_id].marker=m.addTo(markerGroup);
    } else {
      const m=busData[bus.bus_id].marker;
      m.setLatLng([lat,lng]);
      m.setPopupContent(`<strong>Bus ${bus.bus_id}</strong><br>Status: <span style="color:${bus.status==='On Time'?'green':'red'}">${bus.status}</span><br><em>Click marker or row for details</em>`);
    }

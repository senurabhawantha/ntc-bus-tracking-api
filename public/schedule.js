...
function formatTimeOnly(d){return d.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'});}
...
async function fetchRoutes(){
  ...
  const all = document.createElement('option'); all.value='all'; all.textContent='All Routes'; routeSelect.appendChild(all);
  routes.forEach(r=>{const opt=document.createElement('option'); opt.value=r.route_id; opt.textContent=`${r.route_id} — ${r.name}`; routeSelect.appendChild(opt);});
}

function buildScheduledTimes(buses, selectedDate){
  const byRoute = new Map(); buses.forEach(b=>{const r=Number(b.route_id); if(!byRoute.has(r)) byRoute.set(r,[]); byRoute.get(r).push(b);});
  ...
  for(const [routeId,list] of byRoute.entries()){
    ...
    list.forEach((bus,idx)=>{scheduled.set(bus.bus_id,new Date(base.getTime()+idx*45*60*1000));});
  }
  return scheduled;
}

async function loadBuses(){
  busList.innerHTML=''; statusNote.textContent='Loading…'; loadBtn.disabled=true;
  const route = routeSelect.value; const selectedDate = dateInput.value;
  ...
  try{
    ...
    const list=[...buses]; if(route==='all') list.sort((a,b)=>Number(a.route_id)-Number(b.route_id)||Number(a.bus_id)-Number(b.bus_id)); else list.sort((a,b)=>Number(a.bus_id)-Number(b.bus_id));
    list.forEach(b=>{const routeId=Number(b.route_id); const cities=routeCityMap[routeId]||{start:'—',end:'—'}; const startTime=scheduleMap.get(b.bus_id); const timeOnly=startTime?formatTimeOnly(startTime):'—';
      const tr=document.createElement('tr'); tr.innerHTML=`<td>${b.bus_id}</td><td>${routeId}</td><td>${timeOnly}</td><td>${cities.start}</td><td>${cities.end}</td>`; busList.appendChild(tr);
    });
    statusNote.textContent=`Showing ${list.length} buses (${route==='all'?'sorted by route':'for route '+route}).`;
  }catch(e){statusNote.textContent='Failed to load buses.'; console.error(e);}finally{loadBtn.disabled=false;}
}

(async function boot(){await fetchRoutes(); dateInput.value=toYMD(new Date()); await loadBuses(); loadBtn.addEventListener('click',loadBuses);})();
